...
export function NewNoteCart({onNoteCreated}: NewNoteCardProp){
  ...
  function handleStartRecording() {
    ...
    if (!isSpeechRecognitionAPIAvaiable) {
      alert("Infelizmente seu navegador nao suporte a API de agracação")
      return
    }
              ...
              <div className="flex flex-1 flex-col gap-3 p-5">
...

                {
                  shouldShowOnBoarding ? (
                    ...
                    <textarea value={content} onChange={handleContentChanged} autoFocus className="text-sm leading-6 text-slate-400 bg-transparent resize-none flex-1 outline-none"></textarea>
                  )
                }
              </div>
